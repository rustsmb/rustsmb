...
impl AuthProvider for SpnegoProvider {
    fn authenticate<'a>(
        ...
        Box::pin(async move {
            ...
            context.raw_mechanism = true;
            let result = self.inner.authenticate(context, token).await?;

            // For raw mechanism, don't wrap in SPNEGO - return raw response
            // Some clients (e.g., smbtorture) send raw NTLM and expect raw NTLM back
            Ok(result)
        })
    }
